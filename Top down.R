...
TVR_function <- function(dist, n) {
  GM <- solve((-dist@pars$S)) #green matrix
  CV <- dist@pars$alpha %*% GM #contribution vector
  ...
  for (j in 1:4) {
    ...
    for (iter in seq(1, 50000, by = 1)) {
      ...
      if (abs(logLik1 - logLik(fit1)) < change) {
        ...
        ph_model1fin <- TVR_function(fit1,1)
    ...
    for (iter in seq(1, 50000, by = 1)) {
      ...
      if (abs(logLik2 - logLik(fit2)) < change) {
        ...
        ph_model2fin <- TVR_function(fit2,1)
...
file_name <- paste0(folder_path, "resultstopdown.csv")
write.csv(results, file = file_name)
results <- read.csv("C:/Users/bjorn/OneDrive/Desktop/speciale/Final files/Bounds/resultstopdown.csv")
...
#finder færdigt fit i 21 dim
ph_model1 <- ph(structure = "general", dimension = 21)
fit1 <- fit(ph_model1, y = data1, stepsEM = 1)
logLik1 <- logLik(fit1)
for (iter in seq(1, 50000, by = 1)) {
  fit1 <- fit(fit1, y = data1, stepsEM = 1, every = 1)
...
png(filename = "TVR example.png", width = 10, height = 8, units = "in", res = 600)
plot(0, type = "n",
...
lines(sequence, converge21, col = "blue")
lines(sequence, converge20, col = "red")
lines(sequence, first20, col = "grey")
lines(sequence, random20, col = "purple")
legend("bottomright", legend = c("Fit at convergence 21d", "Fit at convergence 20d", "First guess 20d", "Random 20d"),
       col = c("blue", "red", "grey", "purple"), lwd = 2, 
       bty = "n", cex = 1.5)
dev.off()
#Hernæst laves plot hvor vi zoomer ind
setwd("C:/Users/bjorn/OneDrive/Desktop/speciale/Final files/Bounds/Plots")
png(filename = "TVR example zoom.png", width = 10, height = 8, units = "in", res = 600)
plot(0, type = "n",
     xlab = "Data", ylab = "Cumulative Probability", col = "black", 
     xlim = c(0.793725, 0.79373), ylim=c(0.79046, 0.79047))
lines(sort(data1),  empirical1(sort(data1)), col = "darkgreen", type = "s",lwd = 2)
lines(sort(data1), ks_lower_bound1, col = "lightgreen",type = "s", lty = 2)
lines(sort(data1), ks_upper_bound1, col = "lightgreen",type = "s", lty = 2)
lines(sequence, converge21, col = "blue")
...
lines(sequence, first20, col = "grey")
lines(sequence, random20, col = "purple")
legend("topleft", legend = c("Fit at convergence 21d", "Fit at convergence 20d", "First guess 20d", "Random 20d"),
       col = c("blue", "red", "grey", "purple"), lwd = 2, 
       bty = "n", cex = 1.5)
dev.off()
  ...
  for (iter in 1:50000){
    ...
    if (counter2 == 0){
      fit2 <- fit(fit2, y = data1, stepsEM = 1)
        if (abs((logLik(fit2)-logLik2)) < change) {
          iterations2[i] <- iter
          counter2 <- 1
          cdfs2[,i] <- cdf(fit2,sequence)
        } 
      størst2 <- max(abs(logLik(fit2)-logLik2), størst2)
      samlet2 <- samlet2 + abs(logLik(fit2)-logLik2)
      logLik2 <- logLik(fit2)
    }
    if (counter == 1){
      if (counter2 == 1){
...
folder_path <- "C:/Users/bjorn/OneDrive/Desktop/speciale/Final files/Bounds/"
write.csv(cdfs, file = paste0(folder_path, "topdowncdfs.csv"))
write.csv(cdfs2, file = paste0(folder_path, "topdowncdfs2.csv"))
write.csv(iterations, file = paste0(folder_path, "topdowniterations.csv"))
write.csv(iterations2, file = paste0(folder_path, "topdowniterations2.csv"))
cdfs2 <- read.csv("C:/Users/bjorn/OneDrive/Desktop/speciale/Final files/Bounds/bottomupcdfs2.csv")

setwd("C:/Users/bjorn/OneDrive/Desktop/speciale/Final files/Bounds/Plots")
png(filename = "TVR full.png", width = 10, height = 8, units = "in", res = 600)
plot(0, type = "n",
     xlab = "Data", ylab = "Cumulative Probability Top-Down", col = "black", 
     xlim = c(0, max(data1)), ylim=c(0, 1))
lines(sort(data1),  empirical1(sort(data1)), col = "darkgreen", type = "s",lwd = 2)
...
for (i in 1:4) {
  legend_labels[i] <- paste0("TVR fit ",dimensions[i], "d parameters")
}
png(filename = "Random full.png", width = 10, height = 8, units = "in", res = 600)
plot(0, type = "n",
     xlab = "Data", ylab = "Cumulative Probability Random", col = "black", 
     xlim = c(0, max(data1)), ylim=c(0, 1))
lines(sort(data1),  empirical1(sort(data1)), col = "darkgreen", type = "s",lwd = 2)
lines(sequence, pmax(empirical1(sequence) - size, 0), col = "lightgreen",type = "s", lty = 2)
lines(sequence, pmin(empirical1(sequence) + size, 1), col = "lightgreen",type = "s", lty = 2)
...
legend_labels <- character(4)
for (i in 1:4) {
  legend_labels[i] <- paste0("Random fit ",dimensions[i], "d parameters")
}
legend("bottomright", legend = legend_labels, col = rainbow(8), lty = 1, cex = 1)
dev.off()
sum(cdfs2[,2] <= pmin(empirical1(sequence) + size, 1) & cdfs2[,2] >= pmax(empirical1(sequence) - size, 0))
iterations2[2]
